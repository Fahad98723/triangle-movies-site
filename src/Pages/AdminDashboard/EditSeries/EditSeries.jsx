...
const EditSeries = ({ id, setOpenEditModal, setRefetch }) => {
  ...
  useEffect(() => {
    if (singleSeries) {
      setTitle(singleSeries.title);
      setCategory(singleSeries.categories);
      setGenres(singleSeries?.genres);
      setUrl(singleSeries?.url);
      setDetails(singleSeries.overview);
      setCastNames(singleSeries.cast);
      setDirector(singleSeries.director);
      setDriveLink(singleSeries.link);
      setPosterLink(singleSeries.poster);
      setTrailerLink(singleSeries.trailer);
      setSSLinks(singleSeries.screenshots);
      setRating(singleSeries.average_rating);
      setDate(singleSeries.release_date);
      setRuntime(singleSeries.runtime);
      setProductionCompanies(singleSeries.production_companies);
      setProductionCountry(singleSeries.production_countries);
    }
  }, [singleSeries]);
  ...
  const handleSeries = () => {
    ...
    try {
      axios
        .post("http://localhost:5000/api/v1/series/add-series", {
          series: formData,
        })
        .then((res) => {
